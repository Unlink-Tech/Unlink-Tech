...
import { Boxes, Check, ShieldCheck, Wrench } from "lucide-react";
import type { LucideIcon } from "lucide-react";
import { AuroraCard } from "@/components/ui/aurora-card";
import { SectionHeading } from "@/components/section-heading";
import { useInView } from "@/lib/visual-motion";
      ...
      <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 sm:py-24">
        ...
        <div ref={ref}>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:gap-8">
            {ways.map(({ icon: Icon, kicker, lead, rest, body }) => (
              <AuroraCard key={lead}>
          ...
          <div className="relative hidden h-28 md:block" aria-hidden>
            ...
            >
              {/* each route drops, then turns in toward the centre */}
              <span
                className={cn(
              ...
              <span className="absolute bottom-0 left-1/2 top-11 w-px -translate-x-1/2 bg-[linear-gradient(rgb(var(--particle-rgb)/0.3),rgb(var(--particle-rgb)/0.55))]">
                {inView && (
                  <span className="rail-spine-packet absolute left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-[rgb(var(--particle-rgb))] shadow-[0_0_10px_rgb(var(--particle-rgb)/0.85)] motion-reduce:hidden" />
                )}
              </span>
            </div>
          </div>
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:gap-14">
              ...
              <div className="lg:border-l lg:border-border/60 lg:pl-14">
                ...
                <p className="mt-2 max-w-[13rem] text-sm leading-relaxed text-muted-foreground">
                  Reconciliation accuracy, measured on $50M+ daily volume.
                </p>
              </div>
            </div>
          </div>
